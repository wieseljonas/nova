...
async function setupSandboxFilesystem(
  ...
  try {
    ...
    const mountResult = await sandbox.commands.run(
      `sudo chmod 666 /dev/fuse 2>/dev/null; touch /tmp/gcs-sa-key.json && chmod 600 /tmp/gcs-sa-key.json && echo "$GOOGLE_SA_KEY_B64" | base64 -d > /tmp/gcs-sa-key.json && sudo mkdir -p ${mountPath} && sudo chown $(id -u):$(id -g) ${mountPath} && gcsfuse --key-file=/tmp/gcs-sa-key.json --implicit-dirs ${bucketName} ${mountPath}; EXIT=$?; rm -f /tmp/gcs-sa-key.json; exit $EXIT`,
